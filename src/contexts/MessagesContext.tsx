...
import type { Conversation, Message, Seller } from "@/data/mockData";
...
export function MessagesProvider({ children }: { children: ReactNode }) {
  ...
  const startConversation = useCallback(
    (listing: Conversation["listing"], otherUser: Seller, initialMessage: string): string => {
      const existing = conversations.find(
        (c) => c.listing.id === listing.id && c.type === "buying",
      );
      if (existing) {
        const msg: Message = {
          id: `m-${Date.now()}`,
